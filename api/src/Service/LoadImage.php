...
class LoadImage {
    ...
    public function setImage(UploadedFile $image): array {
        ...
        $projectPath = $this->directory . $uniqProject;
        $urlPath = '/images/' . $uniqProject . $file;
        $image->move($projectPath, $image->getClientOriginalName());        
        $array['origin'] = '/images/' . $uniqProject . $image->getClientOriginalName();
        $imagePath = $projectPath . $file;
        switch ($image->getClientOriginalExtension()) {
            case 'jpeg' || 'jpg' || 'JPG' || 'JPEG' || 'png' || 'PNG':
                ...
                $array['big'] = $urlPath . '.webp';
                $array['middle'] = $urlPath . '-900'. '.webp';
                $array['small'] = $urlPath . '-500'. '.webp';
                break;
            case 'webp' || 'WEBP':
                $imagick = new \Imagick($projectPath . $image->getClientOriginalName());
                $imagick->thumbnailImage(900, 900, true);
